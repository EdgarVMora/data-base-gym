...
export default function ListaEmpleados() {
  ...
  async function handleSubmit(e) {
    ...
    setFormLoading(true)
    setFormError(null)
    if (!form.nombre || !form.apellido_paterno || !form.id_puesto || !form.id_tipo_contrato || !form.salario_actual || !form.fecha_contratacion) {
      setFormError('Todos los campos obligatorios')
  ...
  return (
    <section className="scroll-mt-8">
      <SectionIntro
        title="Empleados"
        subtitle="Personal del gimnasio con contrato, puesto y datos personales."
        table="empleados → personas, tipo_contrato, puesto"
      />

      <div className="mb-4 flex justify-end">
        <button
          className="bg-amber-600 hover:bg-amber-700 text-white font-semibold px-4 py-2 rounded shadow"
          onClick={() => { setShowForm(f => !f); setEditId(null); setForm({ nombre: '', apellido_paterno: '', apellido_materno: '', correo_electronico: '', telefono: '', id_puesto: '', id_tipo_contrato: '', salario_actual: '', fecha_contratacion: '' }) }}
        >
        ...
        <form className="mb-6 bg-slate-50 dark:bg-slate-800 p-4 rounded shadow" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input
              ...
              onChange={e => setForm(f => ({ ...f, salario_actual: e.target.value }))}
              required
            />
            <input
              className="border p-2 rounded"
